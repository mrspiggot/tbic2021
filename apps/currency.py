...
layout = html.Div([
    html.H1('TBIC Performance Heatmap by Currency', style={'textAlign': 'center'}),
    dbc.Row([
        dbc.Col(
            dcc.Slider(min=0, max=6, marks=slider_dict, id="ccy-treemap-slider", value=0),
    ...
    dbc.Spinner(dcc.Graph(id='ccy-treemap', figure = {}), color='primary'),
])
@app.callback(Output('ccy-treemap', 'figure'),
              [Input('ccy-treemap-slider', 'value')])
def currency_treemap(value):
    tbic = Portfolio()
    tbic_port = tbic.gain_from_meeting(value)
    tbic_port.round(2)
    tbic_port["portfolio"] = "The TBIC Portfolio - By Currency"
    fig = px.treemap(tbic_port, path=['portfolio', 'currency_x', 'sector', 'industry', 'name'],
